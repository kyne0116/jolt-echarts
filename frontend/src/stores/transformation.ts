...
import type { TransformationResult, TransformationStep } from '@/types'
...
export const useTransformationStore = defineStore('transformation', () => {
  ...
  const updateStepStatus = (stepId: string, status: TransformationStep['status'], data?: any) => {
  ...
  const executeFullTransformation = async () => {
    ...
    try {
      ...
      const templateResponse = await twoStageApi.getTemplate(currentChartId.value)
      ...
      const stage1Response = await twoStageApi.stage1Transform(universalTemplate.value)
      ...
      const stage2Response = await twoStageApi.stage2Transform(currentChartId.value, stage1Output.value)
  ...
  const executeStage1Only = async () => {
    ...
    loading.value = true
    try {
      const response = await twoStageApi.stage1Transform(universalTemplate.value)
  ...
  const executeStage2Only = async () => {
    ...
    loading.value = true
    try {
      const response = await twoStageApi.stage2Transform(currentChartId.value, stage1Output.value)
