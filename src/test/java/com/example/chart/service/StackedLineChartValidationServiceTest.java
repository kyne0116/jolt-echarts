...
public class StackedLineChartValidationServiceTest {
    ...
    @Test
    public void testConfigurationComparison() throws Exception {
        System.out.println("=== 测试配置对比 ===");
        
        // 生成配置
        StackedLineChartValidationService.ValidationResult result = validationService.validateStackedLineChart();
        Map<String, Object> generatedConfig = result.getEchartsConfig();
        ...
        // 对比关键字段
        compareField(originalConfig, generatedConfig, "title");
        compareField(originalConfig, generatedConfig, "xAxis");
        compareField(originalConfig, generatedConfig, "yAxis");
        compareField(originalConfig, generatedConfig, "series");
        
        // 对比系列数量
        List<Map<String, Object>> originalSeries = (List<Map<String, Object>>) originalConfig.get("series");
        List<Map<String, Object>> generatedSeries = (List<Map<String, Object>>) generatedConfig.get("series");
        assertEquals(originalSeries.size(), generatedSeries.size(), "系列数量应该一致");
        
        System.out.println("✅ 配置对比测试通过");
    }

    private void compareField(Map<String, Object> original, Map<String, Object> generated, String fieldName) {
        boolean originalHas = original.containsKey(fieldName);
        boolean generatedHas = generated.containsKey(fieldName);
        
        System.out.println(String.format("字段 %s: 原始=%s, 生成=%s", fieldName, originalHas, generatedHas));
        
        if (originalHas) {
            assertTrue(generatedHas, "生成的配置应该包含字段: " + fieldName);
        }
    }
}
